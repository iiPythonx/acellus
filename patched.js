(function () {
  ...
  window.startVideo = function (tramia, myrtice) {
    ...
    var jhara = false;
    if (tramia.indexOf("progress") >= 0) jhara = true;
    ...
    tyshana = 0;
    najaah = 0;
    var fawn = {fullscreen: {iosNative: true}, captions: {active: false}, autoplay: true, seekTime: 15, vimeo: {title: true}, controls: tramia, speed: {selected: 1, options: [1, 1.25, 1.5, 2]}, settings: ["captions", "quality", "speed"], keyboard: {focused: false, global: false}};
    if (is_playlist) {
      video_player = video_player.children[0];
      ...
      musfirah.attachMedia(video_player);
    }
    ;
    janney = new Plyr(video_player, fawn);
    if (janney.muted) {
      janney.muted = false;
    }
    ;
    ...
    janney.on("timeupdate", function (maxen) {
      if (sullivan && najaah != janney.duration) sullivan = false;
    ...
    janney.on("ready", function () {
      if (document.getElementById("plyr-rewind")) return;
...
function content(answer, tyrise, jaanvi, helpvideopath, answer2) {
  clearInterval(timer);
  if (!resetVideo(tyrise)) return;
  if (answering === false) return;
  Status = VidStatus(tyrise);
  answering = false;
  newMessage = "";
  if (FunctionType !== "library_item") FunctionType = "";
  if (ItemID > 0) FunctionType = "library_item";
  if (requestedVideo !== "") FunctionType = "PlayVideo";
  if (ProblemID !== "" || StepUID !== "" && CoursewarePreview === "Preview") FunctionType = "PreviewProblem";
  if (helpvideopath !== "") FunctionType = "run_help_video";
  $(document).unbind("keyup");
  jQuery.ajax({type: "POST", url: "acellus_functions.php", dataType: "html", async: false, data: {FunctionType: FunctionType, HelpVideoPath: helpvideopath, Answer: answer, Answer2: answer2, VideoDone: Status, Skip: jaanvi, ajax: "true", ClassID: ClassID, CourseID: CourseID, StepUID: StepUID, ItemID: ItemID, Video: requestedVideo, ProblemID: ProblemID, HomeworkID: HomeworkID, CoursewarePreview: CoursewarePreview, AnswerGiven: AnswerGiven, TestMode: testMode, ShowHelp: showHelp}, success: function (rebekkah) {
    ...
    confirmCb("There appears to be an issue with your Internet connection. Make sure you are online and try again.", function (rutva) {
      ...
      ;
      answering = true;
      content(answer, tyrise, jaanvi, "", answer2);
    });
  }});
}
...
function innerContent(mekhiah) {
  ...
  setTimeout("answering = true;", 500);
  startText();
  if (real[24] !== "") showMessage(real[24], damaso);
  if (real[28] !== "") $("#course-content").append(real[28]);
  if (real[5] !== "") {
  ...
  ;
  if (real[6] !== "" && damaso !== "Blockly" && damaso !== "Circuit") startAudio(real[6]);
  if (real[14] !== "") cacheImages(real[14]);
}
...
function saveVideoPosToDatabase(dalayiah) {
  if (!SaveVideoPosition) return;
  if (dalayiah) janney.pause();
  if (navigator.sendBeacon && navigator.userAgent.indexOf("Acellus-iOS") === -1) {
...
function animateClose(markesa, cerra) {
  if (cerra === undefined) cerra = false;
  let kimbra = $("#" + markesa);
  if (cerra) kimbra = $(".menu-open");
...
function openMenu(gianluigi, anjelo) {
  if (anjelo === undefined) anjelo = false;
  ...
  if (branndon.length <= 0) {
    if (gianluigi === "main-resources-menu") {
      ...
      if (gianluigi === "main-help-menu") loadHelpTab();
    }
    return;
  }
  if (ladana) {
